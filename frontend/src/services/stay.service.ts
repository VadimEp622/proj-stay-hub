...
// =============== Checked and in use =============== 
export function query(filterBy: ApiFilterBy): Promise<{ stays: any, isFinalPage: boolean }> {
...
function getStayCategoryScores(stayReviews: Review[]): ReviewScore {
    const scores = stayReviews.reduce((acc: any, review: Review) => {
        Object.entries(review.reviewInputs).forEach(([input, value]: [any, any]) => {
...
function getStayScore(stayReviews: Review[]): number {
    ...
    const stayScoresSum: number = Object.values(scores)
        .reduce((acc: number, value: number) => utilService.floatify(acc += value))
    const stayScore = utilService.floatify(stayScoresSum / Object.values(scores).length)
    return parseFloat(stayScore.toFixed(2))
}

function calculateHowManyNights(checkInDate: number, checkOutDate: number): number {
